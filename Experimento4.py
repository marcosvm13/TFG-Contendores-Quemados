import pandas as pd
    ...
    quemado_predict = []
		...
		with torch.no_grad():
			for images, labels in carga_test:
				...
				if int(outputs[:,0]>0.9): quemado_predict.append(1)
				elif int(outputs[:,0]<0.1): quemado_predict.append(0)
				else: quemado_predict.append(int(not labels[:, 0]))
     return quemado_predict
	...
	def main():
		...
		df = pd.read_csv("datosContenedorGO.csv", usecols= ['FOTO_CONTENEDOR', 'ESTADO_QUEMADO'], sep=';', encoding = 'iso-8859-1')
    df_si = df[df['QUEMADO'].str.match('Si')]
    df_no = df[df['QUEMADO'].str.match('N')].sample(3000)
    df_si = df_s.append(df_s2, ignore_index = True)
		df_si['QUEMADO'].map(lambda x : 1 if x=='Si' else 0)
		...
		quemado_predict = test(modelo, carga_test)
